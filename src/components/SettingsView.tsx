...
export const SettingsView: React.FC = () => {
  ...
  const [pharmacyCouncilRegNo, setPharmacyCouncilRegNo] = useState(profile.pharmacyCouncilRegNo || 'PT-DAR-2024-0891');
  const [supervisingPharmacist, setSupervisingPharmacist] = useState(profile.supervisingPharmacist || 'Dr. Neema Mushi (B.Pharm, MPS)');
  ...
  // In-App Action Modals (safe for Android APK and iframe without window.confirm)
  ...
  const handleSaveBusinessProfile = (e: React.FormEvent) => {
    ...
    updateProfile({
      ...
      enablePharmacyFeatures: bizMode === 'pharmacy' ? true : profile.enablePharmacyFeatures,
    });
  ...
  const executeClearDemo = () => {
    const result = clearDemoData();
    if (result.success) {
      showToast('Data za majaribio zimeondolewa kikamilifu. Mfumo upo tayari kwa matumizi halisi.');
    } else {
      showToast('Haikuweza kuondoa data za majaribio.', true);
    }
  ...
  const executeStartFresh = () => {
    const result = startFreshBusiness();
    if (result.success) {
      showToast('Miamala yote imesafishwa. Mfumo umeanza upya na backup imehifadhiwa.');
    } else {
      showToast('Haikuweza kuanza msimu mpya wa biashara.', true);
    }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
        ...
        <div className="lg:col-span-3">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6">
              ...
              <form onSubmit={handleSaveBusinessProfile} className="space-y-4 text-xs">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {/* Pharmacy / TMDA Specific Credentials */}
                  <div className="sm:col-span-2 p-4 rounded-2xl bg-emerald-950/40 border border-emerald-800/40 space-y-3">
                    <div className="font-bold text-emerald-400 text-xs flex items-center gap-1.5 uppercase tracking-wide">
                      <span>💊 Maelezo ya Usajili wa Baraza la Famasi & TMDA</span>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block font-bold text-slate-300 mb-1 text-[11px]">
                          Namba ya Usajili wa Famasi / Duka la Dawa (Pharmacy Council Reg No)
                        </label>
                        <input
                          type="text"
                          value={pharmacyCouncilRegNo}
                          onChange={(e) => setPharmacyCouncilRegNo(e.target.value)}
                          placeholder="PT-DAR-2024-0891"
                          className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white font-mono focus:border-emerald-500 focus:outline-none"
                        />
                      </div>

                      <div>
                        <label className="block font-bold text-slate-300 mb-1 text-[11px]">
                          Mfamasia Msimamizi (Supervising Pharmacist)
                        </label>
                        <input
                          type="text"
                          value={supervisingPharmacist}
                          onChange={(e) => setSupervisingPharmacist(e.target.value)}
                          placeholder="Dr. Neema Mushi (B.Pharm, MPS)"
          ...
          {activeTab === 'backup' && (
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6">
              ...
              {/* Danger Zone & Data Reset Cards */}
              {isOwnerOrAdmin && (
                <div className="space-y-3 pt-2">
                  <div className="text-xs font-bold text-slate-300 flex items-center gap-2">
                    <Shield className="w-4 h-4 text-amber-400" />
                    <span>Usafi wa Data & Kuanza Biashara Halisi (Data Management)</span>
                  </div>

                  {/* 1. Clear Demo Data Card */}
                  <div className="p-4 rounded-2xl bg-amber-950/30 border border-amber-800/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs">
                    <div>
                      <div className="font-bold text-amber-200 flex items-center gap-1.5">
                        <Sparkles className="w-4 h-4 text-amber-400" />
                        <span>Ondoa Data za Majaribio (Clear Demo Data)</span>
                      </div>
                      <div className="text-amber-300/80 text-[11px] mt-0.5">
                        Huondoa bidhaa zote za mfano, mauzo ya demo na gharama za majaribio, huku ikitunza jina la biashara na watumiaji. Mfumo hutengeneza backup kwanza kiotomatiki.
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={handleClearDemoConfirm}
                      className="px-4 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-500 text-slate-950 font-black text-xs flex items-center gap-1.5 shrink-0 transition active:scale-95 shadow-md shadow-amber-600/20"
                    >
                      <Trash2 className="w-4 h-4" />
                      <span>Ondoa Demo Data</span>
                    </button>
                  </div>

                  {/* 2. Start Fresh Business (Wipe Transactions Only) */}
                  <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs">
                    <div>
                      <div className="font-bold text-white flex items-center gap-1.5">
                        <Package className="w-4 h-4 text-emerald-400" />
                        <span>Anza Upya Msimu wa Biashara (Wipe Transactions Only)</span>
                      </div>
                      <div className="text-slate-400 text-[11px] mt-0.5">
                        Hufuta historia ya mauzo, madeni na matumizi lakini <strong>inabakiza bidhaa zako zote zilizosajiliwa</strong> stoo na bei zao. Inafaa kwa kuanza mwaka mpya wa fedha.
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={handleStartFreshConfirm}
                      className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs flex items-center gap-1.5 shrink-0 transition active:scale-95"
                    >
                      <RefreshCw className="w-4 h-4 text-emerald-400" />
                      <span>Wipe Transactions</span>
                    </button>
                  </div>

                  {/* 3. Reset to Demo Data Card */}
                  <div className="p-4 rounded-2xl bg-red-950/20 border border-red-900/40 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs">
                    <div>
                      <div className="font-bold text-red-200">Rejesha Mfumo wa Demo (Reset to Demo)</div>
                      <div className="text-red-300/70 text-[11px] mt-0.5">
                        Hurejesha bidhaa na takwimu zote za majaribio kama ilivyokuwa siku ya kwanza ya usakinishaji wa mfumo.
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={handleResetDataConfirm}
                      className="px-4 py-2.5 rounded-xl bg-red-950 hover:bg-red-900 border border-red-800 text-red-300 font-bold text-xs flex items-center gap-1.5 shrink-0 transition"
                    >
                      <RefreshCw className="w-4 h-4" />
                      <span>Rejesha Demo</span>
                    </button>
                  </div>
                </div>
              )}
      ...
      {/* 1. Clear Demo Data Modal (In-App) */}
      {showClearDemoModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 sm:p-8 space-y-5 shadow-2xl text-xs">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-black text-base text-white">
                  Ondoa Data za Majaribio (Clear Demo Data)
                </h3>
                <div className="text-[11px] text-slate-400">
                  Andaa mfumo kwa ajili ya biashara yako halisi
                </div>
              </div>
            </div>

            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 text-slate-300">
              <p className="font-bold text-white">Mfumo utafanya yafuatayo:</p>
              <ul className="space-y-1.5 pl-4 list-disc text-[11px]">
                <li>Hutengeneza <strong>backup ya usalama kiotomatiki</strong> kabla ya kufuta chochote.</li>
                <li>Huondoa bidhaa zote za mfano (demo products), mauzo ya majaribio, gharama na madeni.</li>
                <li>Hutunza akaunti zako za watumiaji, majina ya biashara, na neno la siri.</li>
              </ul>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
              ...
              <button
                type="button"
                onClick={executeClearDemo}
                className="px-6 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-xl text-xs font-black shadow-lg shadow-amber-500/20 transition active:scale-95"
              >
                Ndio, Ondoa Demo Data Sasa
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 sm:p-8 space-y-5 shadow-2xl text-xs">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-black text-base text-white">
                  Anza Upya Msimu wa Biashara
                </h3>
                <div className="text-[11px] text-slate-400">
                  Kusafisha miamala yote (mauzo, madeni, gharama)
                </div>
              </div>
            </div>

            <p className="text-slate-300 leading-relaxed">
              Miamala yote ya mauzo, madeni, na gharama zitasafishwa ili kuanza upya. <strong>Bidhaa zote stoo na bei zake zitabaki salama</strong>. Backup ya usalama itahifadhiwa kiotomatiki kwanza.
            </p>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
              ...
              <button
                ...
                className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-black shadow-lg shadow-emerald-600/20 transition active:scale-95"
              >
                Anza Upya Sasa
              </button>
            </div>
      ...
      {/* 3. Reset Demo Data Modal */}
      {showResetDemoModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl text-xs">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-black text-base text-white">
                  Rejesha Mfumo wa Demo?
                </h3>
                <div className="text-[11px] text-slate-400">
                  Hurejesha data za mwanzo za majaribio
                </div>
              </div>
            </div>

            <p className="text-slate-300 leading-relaxed">
              Data zote zilizorekodiwa zitaondolewa na kubadilishwa na data za mfano za usakinishaji.
            </p>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
              ...
              <button
                ...
                className="px-6 py-2.5 bg-rose-600 hover:bg-rose-500 text-white rounded-xl text-xs font-black shadow-lg shadow-rose-600/20 transition active:scale-95"
              >
                Rejesha Demo Sasa
              </button>
            </div>
