...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess }) => {
  ...
  const handleDemoRoleClick = async (role: UserRole) => {
    ...
    const demoPasswords: Record<UserRole, string> = {
      ...
      cashier: 'Cashier1234!',
      waiter: 'Waiter1234!',
      storekeeper: 'Store1234!',
      accountant: 'Manager1234!',
    };

    if (demoUser) {
      const demoPass = demoPasswords[role] || 'Owner1234!';
      setUsernameOrPhone(demoUser.username);
      setPassword(demoPass);
      setErrorMsg(null);
      setLoading(true);

      const res = await loginUser(demoUser.username, demoPass);
      if (res.success) {
        if (res.mustChangePassword && res.user) {
          setMustChangeUser(res.user);
          setOldPasswordForMandatory(demoPass);
          setShowChangePassword(true);
        } else if (onLoginSuccess) {
          onLoginSuccess();
        }
      } else {
        setErrorMsg(res.message || `Imejazwa akaunti ya ${demoUser.name}. Bofya "Ingia Kwenye Mfumo"`);
      }
      setLoading(false);
    }
  };
