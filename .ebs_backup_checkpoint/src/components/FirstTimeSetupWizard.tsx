...
export const FirstTimeSetupWizard: React.FC<FirstTimeSetupWizardProps> = ({ onComplete }) => {
  ...
  const [businessName, setBusinessName] = useState('EBS Smart Biz');
  const [ownerName, setOwnerName] = useState('Selemani Rashid');
  const [phone, setPhone] = useState('0676674705');
  const [email, setEmail] = useState('info@ebsbiz.co.tz');
  const [mkoa, setMkoa] = useState('Dar es Salaam');
  const [wilaya, setWilaya] = useState('Ubungo');
  const [address, setAddress] = useState('Mlimani City, Sam Nujoma Road');
  const [tin, setTin] = useState('134-589-201');
  const [licenseNumber, setLicenseNumber] = useState('BL-DAR-2026-9811');
  const [tagline, setTagline] = useState('Mfumo Mahiri wa Biashara, POS na Usimamizi wa Stoo');
  const [logoPreview, setLogoPreview] = useState<string>('');

  // Step 3: Owner Account
  const [ownerUsername, setOwnerUsername] = useState('elly');
  const [ownerPhone, setOwnerPhone] = useState('0676674705');
  const [password, setPassword] = useState('Owner1234!');
  const [confirmPassword, setConfirmPassword] = useState('Owner1234!');
      ...
      <div className="w-full max-w-3xl bg-slate-950/90 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden backdrop-blur-xl">
        ...
        <div className="bg-gradient-to-r from-emerald-900/60 via-slate-900 to-teal-900/40 p-6 sm:p-8 border-b border-slate-800 relative">
          <div className="flex items-center space-x-3 mb-2">
            <div className="w-10 h-10 rounded-2xl bg-emerald-500 text-slate-950 font-black flex items-center justify-center text-lg shadow-lg shadow-emerald-500/20">
              EBS
            </div>
            <div>
              <span className="text-[11px] font-black uppercase tracking-widest text-emerald-400">
                Enterprise Business System • Tanzania V1.3.0
              </span>
