...
export const AiAssistantView: React.FC = () => {
  ...
  const handleSendMessage = async (customPrompt?: string) => {
    ...
    // Business Data Context to inject into Gemini prompt
    const businessContext = {
      ...
      lowStockProducts: products.filter((p) => p.stockQty <= p.minStock).map((p) => ({
        name: p.name,
        stock: p.stockQty,
        minStock: p.minStock,
      })),
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Hitilafu kwenye mtandao wa AI');
      }
      ...
      const aiMsg: ChatMessage = {
        ...
        sender: 'ai',
        text: data.reply || 'Samahani, sijaweza kupata jibu kwa wakati huu.',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
      ...
      const errorMsg: ChatMessage = {
        id: (Date.now() + 1).toString(),
        sender: 'ai',
        text: 'Samahani, kumekuwa na changamoto ya muunganisho wa AI. Tafadhali hakikisha umeunganisha mtandao au jaribu tena baada ya muda mfupi.',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
